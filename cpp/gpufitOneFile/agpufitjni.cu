...
------------------------------------------------------------------------------------------------------- */
/* -------------------------------------------------------------------------------------------------------
* from models/acf_numerical3d.cuh START
------------------------------------------------------------------------------------------------------- */
//DAN
    ...
    int const num_v_coefs) // NEW
{
   ...
   double volume3d= sqrpi * szeff * 4 * pow(ax*ay, 2)/( (p00/sqrpi * pexpx0 + perfx0) * (p00/sqrpi * pexpy0 + perfy0) );
...
//	double adt = ((double) parameters[1]*x);
    ...
    for (i = 0; i < 6401; ++i)
    {
        ...
        double p0t = ((8*(double) parameters[1]*x) + pow(psfxz1,2) + pow(psfxz2,2))/2;
        ...
        double perfxt =( p1xt * erf(p10xt) )+( p2xt * erf(p20xt) )- (2 * p3xt * erf(p30xt));
        ...
        double perfyt =( p1yt * erf(p10yt) )+( p2yt * erf(p20yt) )- (2 * p3yt * erf(p30yt));
        ...
        double dt2 = (0.25*(pow((zdiff),2)))/(x*sdt*pow((double) parameters[1],2));
...
/*
 ...
 //   double acf1=sum1;
 */
    double acf1 = (sum1*1000000) / (4 * pow(ax*ay, 2)/(volume3d/(sqrpi*sz)));
    double Dpspim = (sumd1*1000000) / (4 * pow(ax*ay, 2)/(volume3d/(sqrpi*sz)));
 ...
 //   double df22 = pow((double) parameters[18], 2) * (1 + (double) parameters[5] - (double) parameters[18] * (double) parameters[5]);
    ...
    double pacf = (1 / (double) parameters[0]) * ((1 - (double) parameters[5]) * (acf1/1.6) + powf((double) parameters[18], 2) * (double) parameters[5] * acf2) / pow(1 - (double) parameters[5] + (double) parameters[18] * (double) parameters[5], 2) * triplet + (double) parameters[4];
...
/*
    ...
    current_derivatives[10 * n_points] = (float)dtripletTtrip * pacf;
  */ 
    current_derivatives[0 * n_points] = (float) (-1 / pow((double)parameters[0], 2)) * (pf1 * (acf1/1.6) + pf2 * acf2) * triplet;
    current_derivatives[1 * n_points] = (1/(double)parameters[0]) * (float)(Dpspim/1.6);
...
/* -------------------------------------------------------------------------------------------------------
* from models/acf_numerical3d.cuh END
------------------------------------------------------------------------------------------------------- */
/* -------------------------------------------------------------------------------------------------------
    ...
    int const num_v_coefs) // NEW
{
    ...
    case ACF_NUMERICAL_3D://DAN
        calculate_acfnumerical3d(parameters, n_fits, n_points, value, derivative, point_index, fit_index, chunk_index, user_info, user_info_size, num_v_coefs);
        break;
    default:
        break;
    }
...
void configure_model(ModelID const model_id, int & n_parameters, int & n_dimensions)
{
    ...
    case ACF_NUMERICAL_3D:       n_parameters = 20; n_dimensions = 1;  break; //DAN
    default:                                                          break;
    }
}
/* -------------------------------------------------------------------------------------------------------

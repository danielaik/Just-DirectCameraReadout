...
//prototype
int setParameter(double exptime, int oW, int oH, int oL, int oT, int incamerabin, int acqmode, int totalframe, int size_b, int arraysize);
...
int setParameter(double exptime, int oW, int oH, int oL, int oT, int incamerabin, int acqmode, int totalframe, int size_b, int arraysize) {
	...
	imgobj.exposureTime_ = exptime * 1000; // milliseconds
...
JNIEXPORT jint JNICALL Java_directCameraReadout_pvcamsdk_Photometrics_1PVCAM_1SDK_setParameterPVCAM(JNIEnv* env, jclass cls, jdouble exposuretime, jint width, jint height, jint left, jint top, jint incamerabin, jint acqmode, jint totalframe, jint size_b, jint arraysize) {

	return setParameter(exposuretime, width, height, left, top, incamerabin, acqmode, totalframe, size_b, arraysize);
}
...
JNIEXPORT void JNICALL Java_directCameraReadout_pvcamsdk_Photometrics_1PVCAM_1SDK_runInfiniteLoopPVCAM(JNIEnv* env, jclass cls, jshortArray outArray, jobject framecounterObj) {
	...
	while (framesAcquired < imgobj.totalframe_) {
		...
		readBufferCpyJNI(frameAddress, imgobj.arraysize_, new_c, env, outArray, framecounterObj, increment_id, carray, isCopy);
